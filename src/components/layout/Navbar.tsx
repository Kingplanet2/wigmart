  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "bg-white/95 backdrop-blur-md shadow-sm" : "bg-white"}`}>
      ...
      <div className="bg-neutral-900 text-white text-center py-2 text-xs tracking-widest font-medium">
        FREE SHIPPING ON ORDERS OVER $150 | USE CODE WIGMART10 FOR 10% OFF
      </div>
  ...
  <span className="text-2xl font-display font-bold text-neutral-900 tracking-tight">
    Wig<span className="text-brand-500">Mart</span>
          ...
          <div className="flex items-center gap-1">
            <Link to="/support" className="hidden md:flex p-2 rounded-full hover:bg-neutral-100 transition-colors">
              <MessageCircle className="w-5 h-5 text-neutral-600" />
            </Link>

            <Link to="/wishlist" className="p-2 rounded-full hover:bg-neutral-100 transition-colors relative">
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white border-t border-neutral-100 px-4 py-4 space-y-3">
          ...
          <Link to="/" onClick={() => setIsMenuOpen(false)} className="block py-2 text-sm font-medium text-neutral-700">Home</Link>
          <Link to="/products" onClick={() => setIsMenuOpen(false)} className="block py-2 text-sm font-medium text-neutral-700">Shop</Link>
          <Link to="/support" onClick={() => setIsMenuOpen(false)} className="block py-2 text-sm font-medium text-neutral-700">Support</Link>
        </div>
      )}
